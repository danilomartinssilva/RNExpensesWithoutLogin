...
import { Text, View, StyleSheet, Platform, ScrollView, ActivityIndicator, FlatList } from 'react-native';
...
export default class List extends Component {
    ...
    componentDidMount() {
        database.orderByChild('date').equalTo(today()).on('value', sn => {
            const expenses = [];
            if (sn.val()) {
                let values = sn.val();
    ...
    _onDeleteItem = (id) => {
        database.child(id).remove().then(s => {
